class EventCountdown extends HTMLElement {
	...
	connectedCallback() {
		...
		if (this.getTimeElement("start") && this.getTimeElement("end")) {
			if (
				new Date(
					this.getTimeElement("end").getAttribute("datetime"),
				).getTime() <
				new Date(this.getTimeElement("start").getAttribute("datetime"))
			) {
				console.error(
					`The \`end\` attribute must represent a date that comes chronologically after the \`start\` attribute!`,
					this,
				);
				return;
			}
		...
		if (this.getTimeElement("start")) {
			this.setAttribute(
				"start",
				this.getTimeElement("start").getAttribute("datetime"),
			);
		}
	...
	getString() {
		...
		if (
			this.start &&
			this.end &&
			this.start.getTime() < nowEpoch &&
			nowEpoch < this.end.getTime()
		) {
	...
	get end() {
		return this.hasAttribute("end")
			? new Date(this.getAttribute("end"))
			: null;
	}

	get annual() {
		return (
			this.hasAttribute("annual") &&
			this.getAttribute("annual") !== "false"
		);
	}
	...
	get formatNumeric() {
		...
		if (this.doubleUp) {
			...
			numericFormat &&
			EventCountdown.numericFormats.includes(numericFormat)
		) {
